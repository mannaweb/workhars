...
function popularityChange(id,popular){
  $.ajax({
    type: 'POST',
    url: BASE_URL+'admin/change-popular-category',
    ...
    success: function (html) {
      ...
      if(data.status == 1){
        if(popular == 1){
          $('#popularity_td_'+id).html('<span class="btn btn-xs btn-success btn-icon-split"><span class="icon text-white-50"><i class="fas fa-check"></i></span><span class="text">Yes</span></span>');
          $('#popularity_a_'+id).html('<i class="fas fa-fw fa-times-circle"></i> Unpopular');
          $('#popularity_a_'+id).removeAttr('onclick');
          $('#popularity_a_'+id).attr('onclick', 'popularityChange('+id+',2)');
        } else {
          $('#popularity_td_'+id).html('<span class="btn btn-xs btn-danger btn-icon-split"><span class="icon text-white-50"><i class="fas fa-times"></i></span><span class="text">No</span></span>');
          $('#popularity_a_'+id).html('<i class="fas fa-fw fa-check-circle"></i> Popular');
          $('#popularity_a_'+id).removeAttr('onclick');
          $('#popularity_a_'+id).attr('onclick', 'popularityChange('+id+',1)');
        }        
        toastr.success(data.msg, 'Changed popularity', {timeOut: 2000,progressBar:true,closeButton:true   });
      } else {
        toastr.error(data.msg, 'Popularity change failed', {timeOut: 2000,closeButton:true,progressBar:true});
      }
    }
  });
}
