...
//  formData.append('description', tinymce.get("description").getContent());
  $.ajax({
    type: "POST",
    url: BASE_URL +'admin/form-save-data',
    data: formData,
    ...
    success:function(result){
      ...
      if(data.status == 1){
        ...
        toastr.success(data.msg, 'Saved', {timeOut: 1000,progressBar:true,onHidden:function() { window.location.href = BASE_URL+'admin/forms'; }   });
      } else {
...
function searchFilter(page_num,loadingType) {
  ...
  $.ajax({
    type: 'POST',
    url: BASE_URL+'admin/form-search-data/'+page_num,
    ...
    success: function (html) {
      ...
      $('#formsList').html(data.html);
      $('#paginationDiv').html(data.pagination);
      if(loadingType != 1){
        $('html, body').animate({scrollTop:$('#formsList').offset().top - 200 }, 500);
      }
...
function deleteConfirm(){
  ...
  $.ajax({
    type: 'POST',
    url: BASE_URL+'admin/delete-form',
...
function statusChange(id,status){
  $.ajax({
    type: 'POST',
    url: BASE_URL+'admin/change-status-form',
...
$(document).ready(function(){
  ...
  var param = url.split('/');
  if( param[param.length - 1] && param[param.length - 1] == 'forms' ){
    searchFilter('',1);
