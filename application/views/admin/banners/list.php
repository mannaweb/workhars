	...
	<div class="card-header py-3 forms">
		<div class="row form-row mb-2">
			<div class="col-sm-6 d-flex">
				<h6 class="title my-auto">Banner list</h6>
			</div>
			<div class="col-sm-6 text-right">
				<?php if(getUserDetails($this->session->userdata('user_id'),'role_manage') && in_array('add-banner', getUserDetails($this->session->userdata('user_id'),'role_manage'))){?>
				<a href="<?php echo base_url()?>admin/add-banner" class="btn btn-sm btn-success" data-tooltip="Add"><i class="fa fa-plus"></i></a>
				<?php } ?>
				...
				<a href="javascript:void(0)" class="btn btn-sm btn-danger" onclick="deleteBanner('all')" data-tooltip="Delete"><i class="fa fa-trash"></i></a>
				<?php } ?>
				<a href="javascript:void(0)" onclick="resetSearch()" class="btn btn-sm btn-primary cancelButton" data-tooltip="Reset"><i class="fa fa-sync"></i></a>
			  <input type="hidden" id="type_id" value="<?php echo $this->input->get('type_id'); ?>">
			</div>
		</div>
				...
				<table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
					<thead>
						<tr>
							...
							<th>
								<a href="javascript:void(0)" class="sorting created_at" onclick="sortBy('created_at','banners.created_at','ASC')">
