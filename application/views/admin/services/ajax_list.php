<?php if($services){ foreach($services as $key=>$val){?>
	...
	<tr>
		...
		<td width="100"><div class="img-container"><a href="<?php echo getserviceImage($val->id,'user_image');?>" class="d-block w-100 h-100" data-fancybox=""><img src="<?php echo getserviceImage($val->id,'user_image');?>" alt=""/></a></div></td>

		<?php if(getUserDetails($this->session->userdata('user_id'),'role_manage') && in_array('edit-service', getUserDetails($this->session->userdata('user_id'),'role_manage'))){?>

            <td><a href="<?php echo base_url()?>admin/edit-service/<?php echo $val->id;?>" target="_blank"><?php echo $val->name;?></a></td>

          

             <td><a href="<?php echo base_url()?>admin/edit-service/<?php echo $val->id;?>" target="_blank"><?php echo date('d M, Y',strtotime($val->created_at));?></a></td>
		...
		<td>
			...
			<div class="dropdown">
				...
				<div class="dropdown-menu dropdown-menu-right">
					...
					<a class="dropdown-item" href="<?php echo base_url()?>admin/edit-service/<?php echo $val->id;?>"><i class="fas fa-fw fa-edit"></i> Edit</a>

					<?php } ?>

					<?php if(getUserDetails($this->session->userdata('user_id'),'role_manage') && in_array('change-status-service', getUserDetails($this->session->userdata('user_id'),'role_manage'))){?>
