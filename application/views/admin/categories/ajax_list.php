<?php if($categories){ foreach($categories as $key=>$val){?>
	<tr>
		...
		<?php if(getUserDetails($this->session->userdata('user_id'),'role_manage') && in_array('edit-category', getUserDetails($this->session->userdata('user_id'),'role_manage'))){?>
            <td><a href="<?php echo base_url()?>admin/edit-category/<?php echo $val->alias;?>" target="_blank"><?php echo $val->name;?></a></td>
             <td><a href="<?php echo base_url()?>admin/edit-category/<?php echo $val->alias;?>" target="_blank"><?php echo date('d M, Y',strtotime($val->created_at));?></a></td>
         <?php } else { ?>
		...
		<td>
			<div class="dropdown">
				...
				<div class="dropdown-menu dropdown-menu-right">
					<?php if(getUserDetails($this->session->userdata('user_id'),'role_manage') && in_array('edit-category', getUserDetails($this->session->userdata('user_id'),'role_manage'))){?>
					<a class="dropdown-item" href="<?php echo base_url()?>admin/edit-category/<?php echo $val->alias;?>"><i class="fas fa-fw fa-edit"></i> Edit</a>
					<?php } ?>
					<?php if(getUserDetails($this->session->userdata('user_id'),'role_manage') && in_array('change-status-category', getUserDetails($this->session->userdata('user_id'),'role_manage'))){?>
					...
					<?php } ?>
					<?php if(getUserDetails($this->session->userdata('user_id'),'role_manage') && in_array('change-popular-category', getUserDetails($this->session->userdata('user_id'),'role_manage'))){?>
					<a class="dropdown-item" href="javascript:void(0)" onclick="popularityChange(<?php echo $val->id;?>,<?php echo ($val->popular == 1)?2:1;?>)" id="popularity_a_<?php echo $val->id;?>"><?php echo ($val->popular == 1)?'<i class="fas fa-fw fa-times-circle"></i>  Unpopular':'<i class="fas fa-fw fa-check-circle"></i>  Popular';?></a>
					<?php } ?>
				...
				<?php } ?>
					...
					<?php if(getUserDetails($this->session->userdata('user_id'),'role_manage') && in_array('delete-category', getUserDetails($this->session->userdata('user_id'),'role_manage'))){?>
