...
class Forms_model extends CI_Model {
	...
	public function getList($params = array(),$limit = array()){
		...
		$this->db->select('*');
		$this->db->from('forms');
		//$this->db->where('forms.deleted_at',NULL);
		if(isset($params['status']) && $params['status']){
			$this->db->where('forms.status',$params['status']);	
		}

		if(isset($params['keyword']) && $params['keyword']){
			$this->db->where('(forms.name LIKE "%'.$params['keyword'].'%")');	
		}
		...
		if(isset($params['startEnd']) && $params['startEnd']){
			//echo 'hi';die;
              ...
              $this->db->where('forms.created >=',$st);
			  $this->db->where('forms.created <=',$et);
		}
	...
	public function saveData($data=array()){
		if($data['id']){
			...
			$this->db->where('id',$id)->update('forms',$data);
		} else {
	...
	public function statusChange($data=array()){
		$this->db->where('id',$data['id'])->update('forms',array('status'=>$data['status']));
	...
	public function deleteData($data=array()){
		...
		if($data['ids']){
			...
			if($data['status'] == 3 ){			
				$getData = $this->db->where_in('id',$ids)->get('forms')->result();
				if($getData){
				...
				$this->db->where_in('id',$ids)->delete('forms');
				return array('status'=>1,'msg'=>'Deleted successfully.');

			}else{
				$this->db->where_in('id',$ids)->update('forms',array('status'=>3));
				return array('status'=>1,'msg'=>'Deleted successfully.');
			}
		} else {
